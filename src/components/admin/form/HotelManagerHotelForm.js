...
import { AddHotel, HotelManagerUpdateHotel } from "../../../graphql/hotel";
...
export default function HotelForm(props) {
  ...
  const [updateHotelInformation] = useMutation(HotelManagerUpdateHotel, {
    context: {
      headers: {
    ...
    onCompleted: (data) => (
      ...
      refetch(),
      NotifyMessage({
        message: data.hotelManagerUpdateHotelInformation.message,
        type: "success",
      })
    ),
    onError: (error) => (
