...
import { getSellerHotels } from "../../redux/actions/HotelActions";
import HotelForm from "../../components/admin/form/NewSellerHotelForm";
import HotelUpdateForm from "../../components/admin/form/sellersHotelUpdateForm";
import {
  Paper,
  makeStyles,
  TableBody,
...
const ManageHotels = () => {
  ...
  const { refetch, loading } = useQuery(ViewHotels, {
    ...
    onCompleted: (data) => dispatch(getSellerHotels(data)),
    onError: (error) => dispatch(getSellerHotels(error)),
  });
  ...
  const [deleteHotel] = useMutation(DeleteHotel, {
    ...
    onCompleted: (data) =>
      NotifyMessage({
        message: data.newSellerDeleteHotel.message,
        type: "success",
      }),
